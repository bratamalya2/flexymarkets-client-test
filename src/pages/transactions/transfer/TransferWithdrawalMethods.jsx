...
function TransferWithdrawalMethods() {
    ...
    return (
        <Stack gap={"1rem"}>
            <Typography variant='h6' fontWeight={"700"} fontSize={"1.5rem"}>Transfer</Typography>
            <Grid container size={12} spacing={4}>
                {
                    transferWithdrawalMethodsData.map((item, i) => (
                        <Grid
                            variant={"section"}
                            ...
                            <Grid>
                                {
                                    Object.entries(item?.details).map(([key, value], i) => (
                                        <Stack key={key} sx={{ flexDirection: "row", gap: ".5rem", alignItems: "center" }}>
                                            <Typography fontSize={"14px"} color='#989b9f'>{key}</Typography>
